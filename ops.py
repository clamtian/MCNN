...
def featurextract(inputs, name = "feaextract"):
    with tf.variable_scope(name):
        inputs1 = tf.nn.relu(conv("front_conv0", inputs, 64, 1, 1))
        inputs2 = tf.nn.relu(conv("front_conv1", inputs1, 64, 3, 1))
        inputs3 = tf.nn.relu(conv("front_conv2", inputs1, 64, 5, 1))
        inputs4 = tf.nn.relu(conv("front_conv3", inputs, 64, 7, 1))
        inputs = tf.concat([inputs1, inputs2, inputs3, inputs4], 3)
        return inputs
...
def conv(name, inputs, nums_out, ksize, strides, padding="SAME", is_SN=False):
    with tf.variable_scope(name):
        W = tf.get_variable("W", shape=[ksize, ksize, int(inputs.shape[-1]), nums_out],
